...
class CSTNormalizer:
    ...
    PATTERN_NODE_TYPES = {
        'call_expression', 'lexical_declaration', 'variable_declaration',
        'arrow_function', 'function_declaration', 'method_definition',
        'assignment_expression', 'member_expression', 'await_expression',
        'for_in_statement', 'for_of_statement', 'if_statement',
        'try_statement', 'return_statement', 'class_declaration',
        'new_expression', 'import_statement', 'export_statement',
    }
    ...
    def _load_categories(self) -> Dict[str, List[str]]:
        ...
        return {
            'DOM_MANIPULATION': ['DOM_OBJECT', 'DOM_METHOD', 'WINDOW_OBJECT', 'querySelector', 'getElementById'],
    ...
    def _handle_member_expression(self, node: Node, level: str) -> str:
        ...
        if prop and level == 'semantic':
            ...
            prop_sig = self.semantic_rules['method'].get(prop_text, self._recursive_normalize(prop, level))
        else:
...
class PatternMiner:
    ...
    def discover_files(self) -> List[Path]:
        ...
        for ext in ['*.js', '*.jsx', '*.ts', '*.tsx', '*.mjs', '*.cjs']:
            for filepath in self.repo_path.rglob(ext):
                ...
                if any(ignored in filepath.parts for ignored in ignore_patterns):
                    continue
    ...
    def mine_repository(self) -> List[CodePattern]:
        ...
        if self.stats['parse_errors'] > 0:
            print(f'⚠️  Files with parse errors (partial extraction): {self.stats["parse_errors"]}')
...
Examples:
  ...
  python enhanced_pattern_miner.py /path/to/repo --output results --min-freq 5
    ...
    parser.add_argument('--format', choices=['json', 'markdown', 'all'],
                       default='json', help='Output format (default: json)')
    parser.add_argument('--max-file-size', type=float, default=2.0,
